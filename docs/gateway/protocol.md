...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.session.attachImage` attaches `{ sessionId, imageBase64, mimeType: "image/jpeg", note? }` to the owning Gateway-relayed realtime provider session without requesting a response. It requires `talk.realtime.allowImageInput=true`, accepts at most 512 KiB decoded JPEGs, and admits at most one frame per 1.5 seconds per session.
    ...
    - `talk.session.submitToolResult` completes a provider tool call emitted by a gateway-owned realtime relay session. Send `{ sessionId, callId, result, options? }`; `result` may be any JSON value. Pass `options: { willContinue: true }` for interim tool output when a final result follows, or `options: { suppressResponse: true }` when the tool result should satisfy the provider call without starting another realtime response.
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to gateway policy. The first supported tool is `openclaw_agent_consult`; clients get a run id plus the execution session key and use that session key for chat lifecycle/history/cancellation while waiting to submit the provider-specific tool result.
    ...
    - `talk.event` is the single Talk event channel for realtime, transcription, STT/TTS, managed-room, telephony, and meeting adapters. A Gateway-relayed provider tool call sends the owning connection `{ relaySessionId, type: "toolCall", itemId, callId, name, args, talkEvent }`; the nested canonical `talkEvent` has `type: "tool.call"`, `sessionId`, `turnId`, `callId`, `itemId`, and `payload: { name, args }`. Configured `talk.realtime.gatewayTools` are executed by the Gateway instead of emitting `toolCall`; the owner receives the resulting nested `tool.result` event for UI parity.
