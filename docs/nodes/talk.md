...
| `realtime.allowImageInput`               | `false`                                    | Enables owner-scoped JPEG attachments for Gateway-relayed realtime sessions. The Gateway accepts at most one decoded 512 KiB frame per 1.5 seconds and adds it to provider context without forcing a response.                                                                                                                                  |
| `realtime.persistTranscript`             | `false`                                    | Persists each completed Gateway-relayed user/assistant voice turn into the configured agent's main session for normal session history and Memory Core recall. Turns that invoke `openclaw_agent_consult` are skipped because the consult already persists them. No audio is stored.                                                             |
...
| `realtime.gatewayTools`                  | -                                          | Gateway-executed function tools for `gateway-relay` sessions. Each tool runs its host-native absolute `exec` path directly without a shell, passes the string at `argKey` (default `command`) as its only argument, and returns stdout to the realtime provider. This is an unsandboxed, trusted-operator surface; built-in names are reserved. |
