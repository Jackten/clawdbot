...
describe("runCodexAppServerAttempt native hook relay", () => {
  ...
  it("keeps parent cleanup and the native hook relay until descendants settle", async () => {
    vi.useFakeTimers();
    try {
      ...
      const deferred = testing.deferCodexNativeDescendantCleanup({
        monitor: {
  ...
  it("retains the relay for a native child even without a separate yield marker", async () => {
    ...
    const startRequest = harness.requests.find((request) => request.method === "thread/start");
    const relayId = extractRelayIdFromThreadRequest(startRequest?.params);
    const generation = extractGenerationFromThreadRequest(startRequest?.params);
    ...
    await expect(
      invokeNativeHookRelay({
        ...
        rawPayload: {
          hook_event_name: "PreToolUse",
          tool_name: "Bash",
          tool_use_id: "detached-child-tool",
  ...
  it("hard-caps a deferred native hook relay at its existing expiry", async () => {
    vi.useFakeTimers();
    try {
      let settleParent: (() => Promise<void> | void) | undefined;
      const unregister = vi.fn();
