...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      const logicalEffectFingerprint = buildMessageToolDeliveryFingerprint({ action, params });
      ...
      if (!actionIdempotencyKey && options?.runId) {
        autogeneratedDeliveryFingerprint = logicalEffectFingerprint;
      ...
      try {
        const runAction = (effectIdempotencyKey?: string) =>
          runMessageActionForTool({
            cfg,
            action,
            params: effectIdempotencyKey
              ? { ...params, idempotencyKey: effectIdempotencyKey }
              : actionParams,
        ...
        if (mutation && isMutatingToolCall("message", { ...params, action })) {
          const resourceKey = createAgentMutationResourceKey("message", {
            accountId,
            channel: params.channel ?? scope.channel,
            target:
              params.target ??
              params.to ??
              params.channelId ??
              effectiveCurrentChannel.currentMessagingTarget,
            threadId: params.threadId ?? currentThreadTs,
            messageId: params.messageId,
            userId: params.userId,
          });
          const effect = await mutation.coordinator.runWithResourceLocks(
            {
            ...
            () =>
              mutation.coordinator.executeExternalEffect({
                jobId: mutation.jobId,
                runId: mutation.runId,
                logicalSlot: `message-tool:${logicalEffectFingerprint}`,
                effectKind: `message.${action}`,
                resourceKey,
                payload: {
                  action,
                  params: stripMessageToolIdempotencyEnvelope(params),
                },
                rejectNewSlotAfterRunChange: true,
                submit: runAction,
                classifySubmitError: (error) => {
                  const detail = formatErrorMessage(error);
                  return error instanceof OutboundDeliveryPreflightError ||
                    isPermanentDeliveryError(detail)
                ...
                reconcile: async (idempotencyKey) => {
                  ...
                  if (outcome.status === "not_sent") {
                    return {
                      ...
                      error:
                        outcome.error ??
                        "The durable delivery queue proves the platform send did not start.",
                    };
                  }
                  if (outcome.status === "missing") {
                    return action === "send" && params.bestEffort === false
                      ? {
                          status: "failed",
                          error:
                            "The required write-ahead delivery queue has no entry, so the platform send did not start. Start a new job to retry.",
                        }
                      : {
                          status: "unknown",
                          error:
                            "No durable queue record exists for this adapter path. Verify the external platform manually before starting a new job.",
                        };
                  }
                  if (outcome.status !== "sent" || action !== "send") {
                    return {
                      status: "unknown",
                      error:
                        outcome.status === "unresolved"
                          ? outcome.error
                          : "No conclusive durable transport reconciliation is available.",
                    };
                  }
                  const retainedResult = outcome.results?.[0];
                  return {
                    status: "applied",
                    value: {
                      kind: "send",
                      ...
                      payload: {
                        status: "reconciled",
                        messageId: outcome.reconciliation?.messageId ?? retainedResult?.messageId,
                        receipt: outcome.reconciliation?.receipt ?? retainedResult?.receipt,
                      },
                      dryRun: false,
          ...
          if (effect.status === "failed") {
            throw new Error(
              `External effect ${effect.idempotencyKey} failed: ${effect.error ?? "unknown error"}`,
            );
          }
          result = effect.value;
        } else {
