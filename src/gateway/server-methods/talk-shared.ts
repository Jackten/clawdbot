...
export function buildTalkRealtimeConfig(config: OpenClawConfig, requestedProvider?: string) {
  ...
  return {
    ...
    gatewayTools: config.talk?.realtime?.gatewayTools,
  };
}
...
const DEFAULT_REALTIME_INSTRUCTIONS = [
  ...
  `When ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} is accepted for background work, speak one brief acknowledgement such as "Let me check that for you", then continue the conversation normally. OpenClaw delivers the terminal result separately.`,
